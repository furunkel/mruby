...
#define SETLOCAL_A(v) (ctx->stack[GETARG_A(I())] = v)
...
#define GETLOCAL_B() (ctx->stack[GETARG_B(I())])
...
OP_FUNC
op_getlocal(struct op_ctx *ctx) {
  SETREG_A(GETLOCAL_B());
}

OP_FUNC
op_setlocal(struct op_ctx *ctx) {
  SETLOCAL_A(GETREG_B());
}
...
_op_stop(struct op_ctx *ctx) {
  ...
  ctx->retval = ctx->stack[ctx->irep->nlocals];
...
_op_rescue(struct op_ctx *ctx, mrb_callinfo *ci) {
  ...
  ctx->stack = MRB()->c->stack = ci[1].stackent;
...
_op_raise(struct op_ctx *ctx, mrb_code *pc) {
  ...
  while (ci[0].ridx == ci[-1].ridx) {
    ...
    if (ci == MRB()->c->cibase) {
      if (ci->ridx == 0) {
        if (MRB()->c == MRB()->root_c) {
          ctx->stack = MRB()->c->stack = MRB()->c->stbase;
          return _op_stop(ctx);
        }
        else {
          struct mrb_context *c = MRB()->c;
...
_op_send_static(struct op_ctx *ctx, mrb_value recv, struct RClass *c,
  ...
  if (MRB_PROC_CFUNC_P(m)) {
    ...
    ctx->stack = MRB()->c->stack = ci->stackent;
    PC_SET(ci->pc);
    cipop(MRB());
  }
  else {
...
op_super(struct op_ctx *ctx) {
  ...
  if (MRB_PROC_CFUNC_P(m)) {
    ...
    /* pop stackpos */
    ctx->stack = MRB()->c->stack = MRB()->c->ci->stackent;
    cipop(MRB());
    PC_INC();
  }
  else {
...
mrb_context_run(mrb_state *mrb, struct RProc *proc, mrb_value self, unsigned int stack_keep)
{
  ...
  ctx.regs = alloca(sizeof(mrb_value) * 16);
